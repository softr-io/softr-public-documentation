...
The **Run Custom Code** action executes **JavaScript** or **Python** inside your workflow. It's the escape hatch for logic that formulas can't express - parsing complex JSON, applying multi-step transformations, or computing values that would require deeply nested branches. The code can also call HTTP APIs with `fetch`, and the integrations you attach to the step authenticate those calls. The full reference is on the [Run Custom Code](/workflows/actions/run-custom-code) page.
