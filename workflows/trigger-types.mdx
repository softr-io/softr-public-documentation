...
Integration triggers connect Softr Workflows to third-party services. Softr Tables, Calendly, Attio, and Zoho CRM are **Instant**, while other services use background polling.
...
**Configuration:** Requires a connected Zoho CRM account. Each trigger watches one module — Leads, Contacts, Accounts, or Deals.

> **Tip:** Zoho CRM triggers are Instant — Zoho notifies Softr as soon as the change happens. The output carries the record ID; **Record updated** also lists the changed fields and their new values. Chain **Get record** to read the full record. A bulk edit or import starts one run per affected record.

**When to use:** Notify a team when a lead is created, act on a deal stage change, or keep your Softr database in step with the CRM when records are deleted.
...
| Record created | Zoho CRM | Instant | Any |
| Record updated | Zoho CRM | Instant | Any |
| Record deleted | Zoho CRM | Instant | Any |
