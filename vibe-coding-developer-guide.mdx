...
The `NavigationAction` component also accepts an optional `recordId?: string` prop. When rendering record-specific links in a list/loop, pass the current record ID so it could be dynamically added as URL param to the final URL `?recordId=<id>`.
