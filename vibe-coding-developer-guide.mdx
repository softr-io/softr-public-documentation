...
Returns `{ action: "OPEN_URL" | "OPEN_PAGE"; destination: string; openIn: "SELF" | "TAB" }`.
...
export default function Block() {
  ...
  return (
    <a
      ...
      target={navigation.openIn === "TAB" ? "_blank" : "_self"}
      rel={navigation.openIn === "TAB" ? "noopener noreferrer" : undefined}
...
export default function Block() {
  ...
  const { options } = useFieldOptions({ select, field: "stockStatus" });
      ...
      <div className="content space-y-4">
        ...
        {items.map((item) => (
          <div key={item.id}>
            ...
            <div>{item.fields.status?.label || "Unknown"}</div>
          </div>
        ))}
      </div>
    </div>
