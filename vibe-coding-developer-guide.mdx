...
When writing to a `PHONE` field, the value must be in unformatted international format: `+` followed by digits only (e.g. `+12125550100`). Some datasources (such as Monday.com) reject values that contain spaces, dashes, or parentheses. Strip all non-digit characters except the leading `+` before submitting:
...
Comments live on records. They're configured once for the whole app in **Settings → Comments**, and the hooks in `@/lib/comments` give you one thread per record. You lay the thread out however the block needs. The app owns its data, its copy and its permissions.
...
Every label, placeholder, empty-state message and the sort direction comes from here. A builder editing them in Settings → Comments expects the change to show up right away, with no republish, so read them from the hook instead of hardcoding strings. Don't expose them as `useTextSetting` either — that's a second source of truth, and it quietly wins over the app's own settings.

```tsx
function useCommentsSettings(options: {
  /** Which layout you're building. The two layouts have separate display settings. */
  displayAs: "BLOCK" | "SIDE_PANEL";
  /** Which datasource the thread is on. Required on a multi-datasource block. */
  from?: string;
}): {
  commentListTitle: string;         // may be empty — render no heading rather than an empty one
  commentBoxPlaceholder: string;
  submitButtonLabel: string;
  deleteButtonLabel: string;
  loadMoreCommentsButtonLabel: string;
  emptyStateMessage: string;        // shown instead of the list when the thread is empty
  notificationsToggleLabel: string;
  sortDirection: "NEWEST_ON_TOP" | "OLDEST_ON_TOP";
  surface: boolean;                 // whether each comment sits on its own card
  showVerticalLine: boolean;        // whether a line joins consecutive comments into one thread
  commentPanelDisplayMode: "PUSH_MAIN_CONTENT" | "COVER_MAIN_CONTENT";  // SIDE_PANEL only
  canPost: boolean;                 // false for a viewer who can read the thread but not post
  quickReactions: Array<{ emoji: string; label: string }>;  // the only emoji this surface can post
  allowMentions: boolean;           // false when the app has mentions turned off
} | null
```

**`displayAs` picks which settings you get.** Settings → Comments holds two independent sets of display options, one per layout. Pass `"BLOCK"` for a thread in the block's own flow, `"SIDE_PANEL"` for a docked panel. Pass the wrong one and the builder's edits look like they do nothing.

**Keep the field names when you destructure.** `commentListTitle` is the "Comment list title" setting, `submitButtonLabel` is "Submit button label". Rename it to `title` and you lose the link to the control the builder edits.
...
```tsx
import { useCommentsSettings } from "@/lib/comments";

export default function Block() {
  const settings = useCommentsSettings({ displayAs: "BLOCK" });

  if (!settings) return null;

  return (
    <section>
      {settings.commentListTitle && <h2>{settings.commentListTitle}</h2>}

      {settings.canPost && (
        <form>
          <textarea placeholder={settings.commentBoxPlaceholder} />
          <button type="submit">{settings.submitButtonLabel}</button>
        </form>
      )}
    </section>
  );
...
The rest of `@/lib/comments` builds the thread itself: `useComments`, `useCommentCreate`, `useCommentDelete`, `useCommentReactions`, `useCommentComposer`, `useMentionSuggestions` and `useCommentSubscription`.
...
```ts
{
  action: "OPEN_URL" | "OPEN_PAGE";
  destination: string;
  openIn: "SELF" | "TAB" | "MODAL";
  modalSize: "SM" | "MD" | "LG" | "XL";  // only used when openIn is "MODAL"
  modalType: "POPUP" | "SLIDING";        // only used when openIn is "MODAL"
}
| { action: "OPEN_CHAT"; actionId: string | undefined }
| { action: "TRIGGER_CUSTOM_WORKFLOW"; actionId: string | undefined }
```

The builder picks the action and how it opens. Your code just passes the value to `NavigationAction` or `navigate()`.

`initialValue` sets the starting point, and takes a looser shape than what you get back:

- Link: `{ destination, action?, openIn?, modalSize?, modalType? }`. `action` comes from `destination` — starts with `/` means `OPEN_PAGE`, anything else `OPEN_URL`. `openIn` defaults to `TAB`.
- Chat or workflow: `{ action: "OPEN_CHAT" }` or `{ action: "TRIGGER_CUSTOM_WORKFLOW" }`. The builder picks which action it points at, so there's nothing else to set.
...
```tsx
import { useNavigation, useNavigationSetting } from "@/lib/editable-settings";
import { useRecordCreate, q } from "@/lib/datasource";
import { Button } from "@/components/ui/button";
...
export default function Block() {
  ...
  const afterSubmit = useNavigationSetting({
    name: "after-submit",
    label: "After Submit",
    initialValue: { destination: "/thank-you", openIn: "SELF" },
  });

  const createRecord = useRecordCreate({
    fields: q.select({ name: "FIELD_ID1" }),
    onSuccess: () => navigate(afterSubmit),
  });

  return <Button onClick={() => createRecord.mutate({ name: "New user" })}>Submit</Button>;
}
```

The second argument takes the same `recordId` and `datasourceId` as `NavigationAction`, for a destination that points at one record:

```tsx
<BarChart
  onBarClick={(bar) =>
    navigate(detailsLink, { recordId: bar.id, datasourceId: "d347bd84-7dc5-4d19-bc98-68417c6ae147" })
  }
/>
```

What `navigate()` does, per setting:

| Setting | What happens |
| --- | --- |
| `OPEN_PAGE` / `OPEN_URL`, `openIn: "SELF"` | Navigates the app there |
| `OPEN_PAGE` / `OPEN_URL`, `openIn: "TAB"` | Opens in a new tab |
| `OPEN_PAGE` / `OPEN_URL`, `openIn: "MODAL"` | Opens in a modal, with the builder's `modalSize` and `modalType` |
| `OPEN_CHAT` | Opens the AI chat for that action |
| `TRIGGER_CUSTOM_WORKFLOW` | Runs the workflow with `recordId` and `datasourceId` |
