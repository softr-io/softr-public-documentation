...
<Tabs>
  ...
  <Tab title="One logo per client company">
    In a client portal you usually store the logo once on a Companies (or Vendors, Suppliers, etc) table and link users to it with a Related Record field. Softr matches the **logged-in user** for the dynamic logo, so the image still has to be readable on the user's own row. Copy the logo image into an image field on the user record — through a [workflow](/workflows/workflows) when a user is created or their company changes, or by hand for a small list. Point the dialog at your users table and map that field.

    <Warning>
      Do not map a **linked record** field, and do not expect a [Lookup](/softr-databases/related-record-lookup-and-rollup-fields) of an image field to work. A linked record field can be selected in the dropdown but holds no image, and a lookup pointing at a File field resolves empty. Either choice leaves users on the default logo.
    </Warning>
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="Only some users get their own logo">
    Those users' rows are missing an image, or they have no row in the mapped table. Users added manually rather than [synced from your data source](/add-and-manage-users/syncing-users-with-a-data-source) are a common cause, since they may not exist in the table you mapped.
  </Accordion>
