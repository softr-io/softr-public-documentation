...
Connecting asks for a user sync, which is what turns the table's rows into app users. One only runs if two-way user sync is switched on for the app, and that is off by default, so a freshly connected table usually shows no users yet. The assistant tells you which happened rather than assuming.
...
Disconnecting is gentler than it sounds, and is the one to reach for if you simply want the app to stop syncing: your existing app users remain and can still sign in, they just stop being updated from the table. New rows stop becoming users, rule-based groups can no longer be written, and the table itself is untouched. Reconnecting the same table restores the connection — though the users only re-sync if two-way sync is on.
