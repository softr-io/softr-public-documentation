...
| Tool | What it does |
|---|---|
| `get_user_connection` | Reads the table that authenticates your end users, and its fields |
...
A rule points at a field on the table that authenticates your end users, using that field's ID rather than its name — which is what `get_user_connection` is for. The same holds for a rule about a dropdown field: it matches the option's ID, not the label you see. Both come back from that one call.
