...
<Frame caption="The SSO handoff between a Softr app and an identity provider.">
  {/* SCREENSHOT TODO: Diagram (not a UI capture). Draw the five-step flow above: user → Softr login page → IdP → signed assertion/token back → Softr session. Label the Softr side "Service provider (SP)" and the other side "Identity provider (IdP)". */}
...
<Steps>
  <Step title="Open your app's authentication settings">
    ...
    <Frame caption="The Authentication screen, where all of an app's login methods are configured.">
      {/* SCREENSHOT TODO: In Studio, open an app → Users → Authentication. Capture the full screen showing Login (Enabled/Disabled), the Authentication method list (Email, Single Sign-on, Google Sign-in, Authenticator app (2FA), Permanent login link), Sign up, Onboarding flows, and Security. */}
  ...
  <Step title="Turn on Single Sign-on">
    ...
    <Frame caption="The Single Sign-on toggle in the Authentication method list.">
      {/* SCREENSHOT TODO: Capture the Authentication method rows with Single Sign-on toggled on, showing both the gear icon and the amber "not yet configured" dot. */}
  ...
  <Step title="Choose your SSO type">
    ...
    <Frame caption="The SSO type dropdown, offering SAML and OpenID.">
      {/* SCREENSHOT TODO: Open the Single sign-on window and click the SSO type dropdown so both options (SAML, OpenID) are visible. */}
  ...
  <Step title="Share Softr's details with your identity provider">
    ...
    <Frame caption="Softr shows the ACS URL and Audience URL to give to your identity provider.">
      {/* SCREENSHOT TODO: Single sign-on window with SSO type = SAML, capturing section "1. Share the following details with your identity provider" with the ACS URL and Audience URL fields and their copy buttons. */}
  ...
  <Step title="Enter your identity provider's details in Softr">
    ...
    <Frame caption="The OpenID fields: Provider URL, Token service URL, Client ID, and Client secret.">
      {/* SCREENSHOT TODO: Single sign-on window with SSO type = OpenID, capturing the Callback URL and the whole "2. Enter the following details from your identity provider" section. */}
...
<Frame caption="The Log in utility page with Continue with SSO enabled.">
  {/* SCREENSHOT TODO: In Studio, open Pages → Utility pages → Log in with SSO enabled. Capture the login card showing "Continue with Google", "Continue with SSO", the Email field, and the Continue button. */}
...
<Frame caption="The Log in flows list in the Sign In block settings.">
  {/* SCREENSHOT TODO: Select the login card on the Log in utility page and capture the right-hand Content panel, focused on the "LOG IN FLOWS" section listing Email, Google Sign-in and Single Sign-on with their visibility (eye) icons. */}
...
<Frame caption="The Users list, with the Auth method column showing how each person signs in.">
  {/* SCREENSHOT TODO: Users → Users. Capture the table showing the Email, Name, Status, User Groups, Creation method and Auth method columns. */}
