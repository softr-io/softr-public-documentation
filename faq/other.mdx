...
Yes, you can build a user-facing CSV importer using the Vibe-coding block. Simply prompt the AI to create a file upload interface that parses CSV data and adds it to your connected data source. This gives your end-users the ability to bulk upload records directly from the published app without needing backend access.
  ...
  <Accordion title="What is DSV2 and what data sources have been upgraded?">
    DSV2 is Softr's enhanced way of connecting to external data sources, which significantly increases the performance of Softr apps.

    All 17 available data sources have now been upgraded (Airtable, Notion, Hubspot, Coda, Clickup, and others) except for the REST API. This means that all of Softr's new features are now compatible with these data sources, including:
    ...
    *   **Multi-sorting**
    ...
    Additionally, speed is improved across the board, including instant user group changes and faster data loading.

    **Note:** For blocks that were connected to Airtable with a specific Airtable View synchronized, the block won't be compatible with DSV2. The recommended approach is to replicate the Airtable view's settings (filters & sorting) directly in the Softr block configuration. Once disconnected from the Airtable view, the block will automatically use DSV2.
  </Accordion>
</AccordionGroup>