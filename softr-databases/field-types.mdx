...
- Purpose: Stores a length of time, rather than a point in time.
- Use Case: Perfect for fields like "Time Logged" or "Estimated Effort."
- Features:
  - Choose which units the value uses — milliseconds, seconds, minutes, hours, or days
  - Minutes and hours are enabled by default, and values are entered and shown as `0h 00m`
  - Can set a default value for newly created records
...
- Purpose: Adds a button to every record that runs a [Softr Workflow](/workflows/workflows) when clicked.
- Use Case: Perfect for row-level actions like "Send Reminder" or "Approve Request."
- Features:
  - Pick an existing workflow, or create a new one straight from the field settings
  - Customizable button label (defaults to "Run")
  - The button stays inactive until a workflow is selected
...
- Features:
  ...
  - Results are **_read-only_** and update automatically.
...
- Features:
  ...
  - **_Read-only_** and cannot be edited.
