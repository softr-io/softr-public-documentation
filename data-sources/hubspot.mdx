...
<Steps>
  <Step title="Connect the HubSpot objects to a Dynamic Block">
    ...
    * **Custom Objects**
    ...
    <Note>
      Some objects appear only when your HubSpot account can share them. Softr hides an object
      when HubSpot does not grant access to it, so the list you see can be shorter than the list
      above. Invoices and Subscriptions, for example, need Commerce Hub.

      If you expect an object and it is missing, reconnect the HubSpot data source and approve
      the access HubSpot asks for. The object appears right after that.
    </Note>
  </Step>
