...
description: "Let the users of your Softr app work with it through their own AI assistant, with the exact same permissions they have in the app."
og:title: "App MCP Server – Softr Help Docs"
og:description: "Let the users of your Softr app work with it through their own AI assistant, with the exact same permissions they have in the app."
...
twitter:description: "Let the users of your Softr app work with it through their own AI assistant, with the exact same permissions they have in the app."
icon: "plug"
---
...
<Note>
  This is different from the workspace-wide [Softr MCP Server](/mcp/overview), which is for **you as the builder** — it reaches your whole workspace, including its databases, apps, and workflows. The **App MCP Server** is for **your app's users**, is scoped to a single published app, uses that app's domain, and has its own OAuth credentials.
</Note>
  ...
  <Tab title="ChatGPT">
    <Steps>
      ...
      <Step title="Create a custom connector">
        Open **Settings → Connectors → Create**. Enter a name, paste the **Server URL** from MCP settings, and leave the Authentication options as-is.
      </Step>
      <Step title="Connect">
        Click **Create**, then **Connect**. ChatGPT opens a browser window to authorize access to your app.
      </Step>
    </Steps>
  </Tab>
...
<AccordionGroup>
  <Accordion title="My client still asks for the Client Secret after I pasted it inline">
    Some CLIs (Claude Code included) treat `--client-secret` as a _prompt-trigger flag_, not a value flag. Any value you append inline is ignored and the CLI prompts interactively. This is intentional so the secret doesn't appear in your shell history. Paste the secret at the prompt instead.
  </Accordion>
